...
export function InformativeBlogTopic({ topic }: { topic: ITopic }) {
	return (
		...
		<div id={topic.id} data-informative-blog-topics>
			...
			<div className="informative-content-blog text mt-8">{topic.content}</div>
				...
				<ul className="mt-8">
					{topic.sub_topics?.map((topic, index) => {
						return (
							<li
								id={topic.id}
								data-informative-blog-topics
								className="informative-content-blog"
								key={`${topic.id}-${index}`}
							>
								<h2 className="text-[22px] font-bold text-[#19002C]">
									{topic.title}
								</h2>
								{topic.content}
								{topic.after}
